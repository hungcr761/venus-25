// Standalone generated kernel for expression 4365
...
__global__ void computeExpression_gen_4365_(StepsParams *d_params, DeviceArguments *d_deviceArgs, ExpsArguments *d_expsArgs, DestParamsGPU *d_destParams)
{
    ...
    while (chunk_idx < nchunks) {
        ...
        if (chunk_idx < k_min_chunk || chunk_idx >= k_max_chunk) {
            eval_expr_4c8a5647<true>(d_params, d_deviceArgs, d_expsArgs, expressions_params, bufferCommitsSize, i);
        } else {
            eval_expr_4c8a5647<false>(d_params, d_deviceArgs, d_expsArgs, expressions_params, bufferCommitsSize, i);
        }
        chunk_idx += gridDim.x;
    }
}
